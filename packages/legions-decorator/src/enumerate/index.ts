...
class EnumMemberClass<
  C extends EnumType = string,
  V extends EnumType = string
> {
  private code: C;
  private value: V;
...
export function EnumDesc(code: EnumType, value: EnumType) {
  const globalClassProperty = ['findCode', 'findValue', 'createOptions'];
  return (target: Object, key: string) => {
    globalClassProperty.map(item => {
      if (!target[item]) {
...
export abstract class AbstractEnumDeclaration extends EnumPlus {}
