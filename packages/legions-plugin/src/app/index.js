import { isObject, onloadScript, checkBrowser } from '../utils';
let microApps = [] // {name:'',entry:'url',container:'DOMid',appId:'',styleId:'',loading:true}
let scriptResources= {} // {'entry':{scripts:[],scriptCache:[],sandbox:[],excludeSandboxFiles:[]}}
let isImportHTML = false;
let externalOnloadScript = []; // [{url:'',code:''}] 已加载过外部资源列表
...
export class MicroApps{
    ...
    mounted(apps = {}) {
        ...
        function main() {
                ...
                res.getExternalScripts().then(exports => {
                    if (res.getScripts) {
                        ...
                        excludeFiles.forEach((item) => {
                            ...
                            if (!externalOnloadScript.length) { // 如果外部加载资源项为空，则直接加载资源
                                fn()
                            } else {
         ...
         let wrap = document.getElementById(container)|| document.getElementsByClassName(container);
        ...
        if (window.importHTML&&isImportHTML) {
            ...
            if (window.importHTML) {
                ...
                for (var i=0; i< scripts.length;i++) {
                    ...
                    if (script && script.getAttribute("src") && script.getAttribute("src").indexOf("https://hoolinks.com/static/common/plugins/import-html-entry.js")>-1) {
