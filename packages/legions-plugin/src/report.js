import { assignObject, serializeObj, getNetworkType, BrowserMatch } from './utils';
...
export class Report extends Events{
    ...
    watch(appSystem = '',environment = '') {
        ...
        window.addEventListener("unhandledrejection", function(e){ // Promise Catch 捕获异常
            e.preventDefault()  // 控制台不显示错误信息
            console.log('捕获到异常：', e);
            /* return true; */
        });
        /* performance.getEntries().forEach(function(item){console.log(item.name)}) */
        window.addEventListener('error', (error) => {  // 图片资源及脚本请求异常上报
