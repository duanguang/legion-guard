...
(function (global, factory) {
  ...
  function setHeaders(options) {
      invariant.invariant(typeof options === 'object', 'options: options should be a object');
      options = options || {};
      options.url = options.url || '';
      invariant.invariant(options.url, 'options.url: options.url should not a empty');
      return {
          'api-target': options.url,
      };
  }
  function get(url, prams, options) {
      if (prams) {
          // @ts-ignore
          prams._ = Date.parse(new Date()); // 设置请求不缓存
      }
      var headers = Object.assign(DefaultOption, options);
      return axios$1
  ...
  function post(url, prams, options) {
      var headers = Object.assign(DefaultOption, options);
      return axios$1
