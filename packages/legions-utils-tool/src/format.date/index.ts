 ...
 * @param {Object} date
 */
export function formatDateToFriendly(date: Date) {
  date = date || new Date();
  date =
    typeof date === 'number' || typeof date === 'string'
      ? new Date(date)
...
export function formatDurationToFriendly(second: number) {
  invariant(typeof second === 'number', 'second: object type should is number');
...
export function formatTimeToFriendly(second: number) {
...
export function isLeapYear(year: Date | number) {
 ...
 */
export function getMonthDays(date?: Date | number, month?: number) {
  var y, m;
 ...
 * @param date2 日期二
 */
export function countDays(date1: Date | string, date2: Date | string) {
  var fmt: any = 'yyyy-MM-dd';
 ...
 * @returns
 */
export function getLastDay(date: Date | number | string) {
  date = date || new Date();
  date =
    typeof date === 'number' || typeof date === 'string'
      ? new Date(date)
